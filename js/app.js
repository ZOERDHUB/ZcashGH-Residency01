...
(() => {
  ...
  /** Shows exactly one of the three steps, hiding the other two. */
  ...
  /**
   * Shows either the "confirm this" checkbox+button, or the
   * already-confirmed card — reflecting PB_TRANSACTION's actual
   * confirmed state rather than any local flag, so returning to this
   * step after editing the amount or recipient (which auto-unconfirms
   * in transactionState.js) always shows the correct one.
   */
  function renderReviewConfirmState() {
    if (PB_TRANSACTION.isReviewConfirmed()) {
  ...
  els.confirmReviewBtn.addEventListener("click", () => {
    if (PB_TRANSACTION.confirmReview()) {
      renderReviewConfirmState();
    }
  });

  els.unconfirmReviewBtn.addEventListener("click", () => {
    // "Make changes" doesn't discard anything by itself — it just
    // re-opens the confirm gate. Actually changing the amount or
    // recipient (via the edit buttons below) is what invalidates the
    // confirmation in transactionState.js; this alone does not.
    els.reviewConfirmedCard.classList.add("d-none");
    els.reviewConfirmSection.classList.remove("d-none");
    els.reviewAckCheckbox.checked = false;
    els.confirmReviewBtn.disabled = true;
