...
test("creates an order with all required fields", () => {
  ...
  assert.ok(order.depositAddress && order.depositAddress.startsWith("zs1"));
});

test("the deposit address is tied to this specific order's id (same id, same address)", () => {
  const order = PB_ORDER_STORE.createOrder(validPayload());
  const expected = PB_DEPOSIT_ADDRESS.generateDepositAddress(order.id);
  assert.strictEqual(order.depositAddress, expected);
});

test("getOrder and getMaskedOrder both pass the deposit address through unchanged", () => {
  const created = PB_ORDER_STORE.createOrder(validPayload());
  assert.strictEqual(PB_ORDER_STORE.getOrder(created.id).depositAddress, created.depositAddress);
  assert.strictEqual(PB_ORDER_STORE.getMaskedOrder(created.id).depositAddress, created.depositAddress);
});
